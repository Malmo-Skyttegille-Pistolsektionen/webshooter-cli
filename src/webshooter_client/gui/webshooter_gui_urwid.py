...
class WebShooterGUI:
  ...
  def command_chosen(choice, button):
    ...
    if args['command'] not in ['competitions']:
      main.original_widget = urwid.Padding(WebShooterGUI.menu(f"{command_to_string(args['command'])}", ['card', 'club'], WebShooterGUI.card_club_to_string, WebShooterGUI.user_chosen), left=2, right=2)
    else:
  ...
  def user_chosen(choice, button):
    ...
    main.original_widget = urwid.Padding(WebShooterGUI.menu(f"{command_to_string(args['command'])}, {WebShooterGUI.card_club_to_string(args['user'])}", years, None, WebShooterGUI.year_chosen), left=2, right=2)
  ...
  def year_chosen(choice, button):
    ...
    if args['command'] not in ['medals', 'starts', 'competitions']:
      ...
      main.original_widget = urwid.Padding(WebShooterGUI.menu(f"{command_to_string(args['command'])}, {WebShooterGUI.card_club_to_string(args['user'])}, {args['year']}", c, WebShooterGUI.competition_to_string, WebShooterGUI.competition_chosen), left=2, right=2)
  ...
  def run(webshooter_rc: WebShooterRC) -> "WebShooterGUI":
    ...
    global main
    main = urwid.Padding(WebShooterGUI.menu("Mode", ["signups", "starttimes", "ical", "results", "medals", "starts", "competitions"], command_to_string, WebShooterGUI.command_chosen), left=2, right=2)
    top = urwid.Overlay(
          main,
          urwid.SolidFill(),
          align=urwid.CENTER,
